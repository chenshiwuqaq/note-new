...
public interface TreeNodeMapper {
    ...
    List<TreeRelation> findByAncestor(@Param("nodeId") long nodeId);

    @Select("SELECT * FROM tree_relation WHERE descendant = #{nodeId}")
    List<TreeRelation> findByDescendant(@Param("nodeId") long nodeId);
    ...
    String SearchLabelById(@Param("nodeId") long nodeId);
    ...
    boolean deleteNodeRelation(@Param("nodeId")long nodeId);
    @Delete("DELETE FROM tree_nodes WHERE node_id = #{nodeId}")
    boolean deleteNodeById(@Param("nodeId")long nodeId);
}
